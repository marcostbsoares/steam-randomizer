...
async function getRandomizeAsync(req, res) {
    ...
    try {
        ...
        if (userID !== null) {
            let game = await requestGamesList(userID, maxPlaytime);
...
async function requestGamesList(finalID, maxPlaytime) {
    ...
    data.response.games.forEach(function (game) {
        ...
        gamesList.push(Number(game.appid));
    });

    id = gamesList[Math.floor(Math.random() * gamesList.length)]

    if(id != undefined)
    {
        let result; 
        await Games.findOne({ "app_id": id }, function(err, res){
            result = res;
        });
